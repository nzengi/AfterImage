...
Air-gap Solana transaction signing: both single-signer and **M-of-N
...
#### M-of-N multi-signature flow

```
 ┌──────────────────────────────────────────────────────────────────────┐
 │                       ONLINE MACHINE                                 │
 │  afterimage multisign init tx.bin                                    │
 │    --signers A,B,C --threshold 2 → round1.json                       │
 │  afterimage send round1.json                                         │
 └────────────────────────────┬─────────────────────────────────────────┘
          QR stream (encrypted)│
                               ▼
 ┌──────────────────────────────────────────────────────────────────────┐
 │               AIR-GAPPED MACHINE — Signer A                         │
 │  afterimage recv round1.json                                         │
 │  afterimage multisign sign round1.json --keypair id.json             │
 │    → resp1.json                                                      │
 │  afterimage send resp1.json                                          │
 └────────────────────────────┬─────────────────────────────────────────┘
          QR stream (encrypted)│
                               ▼
 ┌──────────────────────────────────────────────────────────────────────┐
 │                       ONLINE MACHINE                                 │
 │  afterimage recv resp1.json                                          │
 │  afterimage multisign next resp1.json --request round1.json          │
 │    → round2.json                                                     │
 │  afterimage send round2.json                                         │
 └────────────────────────────┬─────────────────────────────────────────┘
          QR stream (encrypted)│
                               ▼
 ┌──────────────────────────────────────────────────────────────────────┐
 │               AIR-GAPPED MACHINE — Signer B                         │
 │  afterimage recv round2.json                                         │
 │  afterimage multisign sign round2.json --keypair id.json             │
 │    → resp2.json   ← threshold met; complete=true                     │
 │  afterimage send resp2.json                                          │
 └────────────────────────────┬─────────────────────────────────────────┘
          QR stream (encrypted)│
                               ▼
 ┌──────────────────────────────────────────────────────────────────────┐
 │                       ONLINE MACHINE                                 │
 │  afterimage recv resp2.json                                          │
 │  Decode signed_transaction_b64 → submit to cluster ✓                │
 └──────────────────────────────────────────────────────────────────────┘
```

**Security properties of the multi-sig workflow:**

- No private key material ever crosses the air gap — only public signature
  bytes travel in the response.
- Each air-gapped signer verifies all prior partial signatures before
  adding its own (chain-of-custody check).
- A 32-byte random nonce embedded in the round-1 request prevents replay
  attacks across sessions.
- Each round specifies the exact pubkey expected to sign it; presenting the
  wrong keypair is rejected immediately.

Build the CLI with multi-sig support:

```bash
cargo build --release -p afterimage --features solana
```
